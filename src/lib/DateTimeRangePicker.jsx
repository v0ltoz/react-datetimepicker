...
class DateTimeRangePicker extends React.Component {
    ...
    dateSelectedNoTimeCallback(cellDate) {
        if (this.props.maxDays != null && this.props.maxDays === 1) {
            ...
            if (this.props.useVirtualSelection && this.state.selectingModeFrom) {
                endToUse = new startToUse.clone();
            }

            let newDates = datePicked(startToUse, endToUse, cellDate, this.state.selectingModeFrom);
    ...
    cellDateHover(date, isFrom) {
        console.log("hover on " + date.toString() + " mode: " + isFrom);
        if (this.props.useVirtualSelection) {
            if (this.state.selectingModeFrom) {
                ...
                console.log("set end date " + date.toString());
                this.setState({
                    end: date
                });

                this.updateStartEndAndLabels(this.state.start, date);
            }
        }
    }
        ...
        return (
            <Fragment>
                <Ranges
                    ...
                    calendarStyles={this.props.calendarStyles}
                />
                {this.props.maxDays == null || this.props.maxDays > 1 /* && !this.props.useVirtualSelection  */ ? this.renderStartDate() : ""}
                {this.renderEndDate()}
            </Fragment>
        );
    }

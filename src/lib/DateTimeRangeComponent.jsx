...
export default class DateTimeRangeComponent extends Component {
  ...
  componentWillMount() {
    ...
    let rangesStr =
      this.props.translations != null
        ? this.props.translations.DefaultRanges
        : null;
    ...
    if (this.props.ranges == null) {
      ...
      if (maxDays === 1)
        ranges[rangesStr[2]] = [
          moment(start).subtract(2, "days"),
          moment(end).subtract(2, "days")
        ];

      if (maxDays > 3)
        ranges[rangesStr[3]] = [moment(start).subtract(3, "days"), moment(end)];
      if (maxDays > 5)
        ranges[rangesStr[4]] = [moment(start).subtract(5, "days"), moment(end)];
      if (maxDays > 7)
        ranges[rangesStr[5]] = [moment(start).subtract(7, "days"), moment(end)];
      if (maxDays > 14)
        ranges[rangesStr[6]] = [
      ...
      if (maxDays > 90)
        ranges[rangesStr[8]] = [
          moment(start).subtract(3, "months"),
          moment(end)
        ];
      if (maxDays > 365)
        ranges[rangesStr[9]] = [
  ...
  applyCallback(startDate, endDate) {
    ...
    returnValue.from_str = moment(returnValue.from).format(
      "YYYY-MM-DD HH:mm:ss"
    );
    ...
    return (
      ...
      >
        <DateTimeRangeContainer
          ...
          disableTime={
            this.props.enableTime == null || this.props.enableTime === false
          }
        ...
        >
          <div className="period-selector">
            ...
            <input
              ...
              type="text"
              label="Text"
              value={this.state.selectedTextFrom}
            />
            {this.props.maxDays === 1 ? (
              ...
              <input
                ...
                type="text"
                label="Text"
                value={this.state.selectedTextTo}
              />
            )}
          </div>
        </DateTimeRangeContainer>
...
const DefaultRangesDescriptors = [
  "Today",
  "Yesterday",
  "Two days ago",
  "3 days",
